...
export class AdminEffects {
  ...
  getUserProjects$ = this.actions$.pipe(
    ...
    mergeMap((payload: any) =>
      this.adminService.getUserProjects(payload.uid).pipe(
        map((data: any) => {
          ...
          return new fromAdmin.UserProjectsLoaded({
            uid: payload.uid,
  ...
  deleteUserProject$ = this.actions$.pipe(
    ...
    switchMap((payload: any) =>
      this.adminService
        .deleteUserProject(payload.userId, payload.projectId)
        .pipe(
          catchError((error: any) => of(new fromAdmin.AdminError({ error })))
        )
    )
  ...
  getUserCustomers$ = this.actions$.pipe(
    ...
    mergeMap((payload: any) =>
      this.adminService.getUserCustomers(payload.uid).pipe(
        map((data: any) => {
          ...
          return new fromAdmin.UserCustomersLoaded({
            uid: payload.uid,
            userCustomers: customersData,
          });
        }),
        catchError((error) => of(new fromAdmin.AdminError({ error })))
      )
