...
export class CustomersEffects {
  ...
  query$ = this.actions$.pipe(
    ...
    switchMap(([, user]: any) =>
      this.customersService.get(user.uid).pipe(
        map((data: any) => {
          ...
          return new fromCustomers.CustomersLoaded({
  ...
  added$ = this.actions$.pipe(
    ...
    switchMap(([payload, user]: any) =>
      this.customersService.add(payload.customer, user.uid)
    )
  );
  ...
  edit$ = this.actions$.pipe(
    ...
    withLatestFrom(this.store.pipe(select(getUser))),
    switchMap(([payload, user]: any) =>
      this.customersService.update(payload.customer, user.uid).pipe(
        catchError((error) => {
          return of(new fromCustomers.CustomersError({ error }));
        })
      )
  ...
  delete$ = this.actions$.pipe(
    ...
    withLatestFrom(this.store.pipe(select(getUser))),
    switchMap(([payload, user]: any) =>
      this.customersService.delete(payload.customer, user.uid)
    )
  );
}
